...
def parse_cbao(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Libellé : x0 ≈ 150–370
            label_words = [w for w in row if 150 <= w['x0'] < 370]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            if not label or any(s in label_up for s in SKIP):
                i += 1; continue
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text']) for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 150 <= w['x0'] < 370).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j
...
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'VALEUR', 'DESCRIPTION', 'RÉFÉRENCE', 'REFERENCE',
            ...
            'BMCE', 'FRAIS', 'TAXE SUR', 'DROIT TIMBRE', 'COMMISSION'}
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Montants débit/crédit
            debit_words  = [w for w in row if 490 <= w['x0'] < 560]
            credit_words = [w for w in row if 560 <= w['x0'] < 630]
            ...
            # debit_raw négatif = crédit (remboursement)
...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    full_text = ' '.join(pages_text)
    m_bal = re.search(r'SOLDE\s+([\d\s]+)$', full_text.upper(), re.MULTILINE)
    if not m_bal:
        m_bal = re.search(r'Solde\s+D[ée]but\s+P[ée]riode\s+([\d\s]+)', full_text, re.IGNORECASE)
    # Balance from text: "SOLDE 1 136 108" at end
    for pat in [
        r'SOLDE\s+([\d][\d\s]{3,})\s*$',
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Détails Transaction : x0 ≈ 80–380 (en excluant agence numérique et date valeur)
            label_words = [w for w in row if 80 <= w['x0'] < 280
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
            ...
            # Mouv. Débit : x0 ≈ 390–475
            debit_words  = [w for w in row if 390 <= w['x0'] < 475
                            and re.search(r'\d', w['text'])]
            # Mouv. Crédit : x0 ≈ 475–560
            credit_words = [w for w in row if 475 <= w['x0'] < 560
                            and re.search(r'\d', w['text'])]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)
            ...
            while j < len(rows):
                ...
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                               for w in r2):
                    break
                nl_words = [w for w in r2 if 80 <= w['x0'] < 280
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                            and not re.match(r'^\d{3,4}$', w['text'])]
