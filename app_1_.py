        ...
        while i < len(rows):
            ...
            if len(resolved) == 0:
                ...
                if is_credit and not is_debit:
                    ...
                    # Position : > 500 = crédit probable, sinon débit
                    if x0 > 500:
                        credit_amt = val
                    else:
                        debit_amt = val
            elif len(resolved) >= 2:
                # Exclure le solde (le plus à droite si >= 3 blocs)
...
#   Débit       : x0 ≈ 490–560  (montants négatifs dans la colonne Débit → crédit)
#   Crédit      : x0 ≈ 560–630
#   Solde       : x0 ≥ 630 (ignoré)
# Particularité : certains montants sont négatifs (ex: -200 000,00) → inversement sens
# ════════════════════════════════════════════════════════════════════════════
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # BOA Villa Yemaya : Débit x0 ≈ 490-560, Crédit x0 ≈ 560-630
            # Les montants négatifs dans la col Débit = crédit (remboursement)
            debit_words  = [w for w in row if 490 <= w['x0'] < 570]
            credit_words = [w for w in row if 570 <= w['x0'] < 650]
            ...
            # Détermination sens débit/crédit :
            # - Montant en col débit négatif → c'est un crédit (remboursement)
            # - Montant en col crédit positif → crédit
            # - Montant en col débit positif → débit
            # - Si aucune position claire → fallback sémantique
            if debit_raw is not None:
                if debit_raw < 0:
                    # Montant négatif dans colonne débit = entrée d'argent (crédit)
                ...
                # Fallback : chercher n'importe quel montant à droite
                all_right = [w for w in row if w['x0'] >= 490]
                amt = _boa_parse_amount_raw(all_right)
                if amt is not None:
                    is_credit = any(k in label_up for k in CREDIT_KW)
                    is_debit  = any(k in label_up for k in DEBIT_KW)
                    if is_credit and not is_debit:
                        txns.append(_make_txn(date_ofx, abs(amt), name, memo))
                    else:
                        txns.append(_make_txn(date_ofx, -abs(amt), name, memo))
...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Mouv. Débit : x0 ≈ 380–480  (élargi)
            debit_words  = [w for w in row if 380 <= w['x0'] < 480
                            and re.search(r'\d', w['text'])]
            # Mouv. Crédit : x0 ≈ 480–570  (élargi)
            credit_words = [w for w in row if 480 <= w['x0'] < 570
                            and re.search(r'\d', w['text'])]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)
            ...
            if debit_amt and debit_amt > 0:
                ...
                # Fallback sémantique : chercher tout montant à droite du libellé
                all_right = sorted([w for w in row if w['x0'] >= 380
                                    and re.search(r'\d', w['text'])],
                                   key=lambda w: w['x0'])
                if all_right:
                    # Regrouper en blocs
                    ...
                    # Prendre le premier bloc comme montant principal (ignorer solde)
                    amt = None
                    for b in blocs[:2]:
                        v = _uba_join_amount(b)
                        if v and v > 0:
