...
def _uba_join_amount(words):
    """
    Reconstitue un montant XOF fragmente en plusieurs tokens pdfplumber.
    Ex : ['41', '295,00'] → '41 295,00' → 41295.0
    Ex : ['2', '316', '032,00'] → '2 316 032,00' → 2316032.0
    """
    if not words:
        return None
    full = ' '.join(w['text'] for w in words).replace('\xa0', ' ').strip()
...
def parse_bsic(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        for row in rows:
            ...
            # Débit (x0 310-440) / Crédit (x0 430-560)
            # Note: _uba_join_amount gère les montants fragmentés "200 497" → 200497
            debit_words  = [w for w in row if 310 <= w['x0'] < 440]
            credit_words = [w for w in row if 430 <= w['x0'] < 560]
...
def parse_uba(pages_words, pages_text, _pdf_path=''):
    ...
    # Mots-clés de crédit et débit pour l'heuristique UBA
    CREDIT_KW = {'MAINLEVEE','BLOCAGE','RECU','SWIFT','VIREMENT OFMS','DECAISSEMENT',
                 'VERSEMENT','REMISE','CREDIT'}
    ...
    for pw in pages_words:
        ...
        for row in rows:
            ...
            # (on exclut la zone date-valeur ~340-420)
            label_words = [w for w in row if 80 <= w['x0'] < 350]
            label = ' '.join(w['text'] for w in label_words).strip()
            if not label or any(s in label.upper() for s in SKIP):
                continue

            # Récupérer tous les mots numériques à droite du libellé
            # UBA a typiquement : [date_valeur] [débit] [crédit] [solde]
            # Les positions varient selon le PDF, on détecte via le Solde (dernier montant)
            right_words = sorted([w for w in row if w['x0'] >= 340], key=lambda w: w['x0'])

            # Regrouper les tokens numériques contigus en "blocs montants"
            # Un bloc = séquence de tokens dont le premier commence par un chiffre
            # et dont chaque token est à moins de 35px du précédent
            def _group_number_tokens(words):
            ...
            blocks = _group_number_tokens(right_words)
            # On s'attend à 1-4 blocs : date_valeur?, débit?, crédit?, solde?
            # Le solde est toujours présent → dernier bloc
            # Si 1 bloc = seul solde → pas de mouvement (ligne de solde initial)
            # Si 2 blocs = [valeur, solde] ou [montant, solde] → 1 mouvement
            # Si 3 blocs = [valeur, montant, solde] → 1 mouvement
            # Si 4 blocs = [valeur, débit, crédit, solde] → débit OU crédit

            # Filtrer les blocs qui ressemblent à une date (dd/mm/yyyy déjà parsée, ou date valeur)
            # Un bloc "date valeur" est de type "dd/mm/yyyy" → déjà exclu car x0<340
            # Mais certains ont x0 ~340-420 et sont de format "02/09/2024" → à exclure
            numeric_blocks = []
            for b in blocks:
                joined = ''.join(w['text'] for w in b)
                if re.match(r'^\d{2}/\d{2}/\d{4}$', joined):
                    continue  # date valeur, on skip
            ...
            # Logique de déduction débit/crédit
            debit_amt = None
            credit_amt = None

            nb = len(numeric_blocks)
            if nb == 1:
                # Seul un montant (= solde ou unique mouvement) → heuristique libellé
                val, x0 = numeric_blocks[0]
                lup = label.upper()
                is_credit = any(k in lup for k in CREDIT_KW)
                is_debit  = any(k in lup for k in DEBIT_KW)
                if is_credit and not is_debit:
                    credit_amt = val
                elif is_debit and not is_credit:
                    debit_amt = val
                ...
                # [montant, solde] → 1 seul mouvement, on utilise l'heuristique
                val, x0 = numeric_blocks[0]
                lup = label.upper()
                is_credit = any(k in lup for k in CREDIT_KW)
                is_debit  = any(k in lup for k in DEBIT_KW)
                if is_credit and not is_debit:
                    ...
                    # Pas de mot-clé → regarder la position x du montant
                    # Débit généralement à gauche du crédit dans le tableau
                    credit_amt = val  # défaut : crédit (mouvement entrant)

            elif nb >= 3:
                # [date_val?, débit_ou_0, crédit_ou_0, solde]
                # On prend l'avant-dernier (montant) et l'anté-avant-dernier
                # Le vrai débit/crédit est dans les 1 ou 2 blocs avant le solde
                amounts = [v for v, _ in numeric_blocks]
                x0s     = [x for _, x in numeric_blocks]
                # Heuristique position : débit est à gauche du crédit
                # Dans UBA, Débit ≈ x0 420-510, Crédit ≈ x0 510-620
                if nb == 3:
                    # [valeur_ou_débit, crédit_ou_débit, solde]
                    v1, x1 = numeric_blocks[0]
                    v2, x2 = numeric_blocks[1]
                    if x2 > x1 + 50:
                        # v1 = débit zone, v2 = crédit zone
                        debit_amt  = v1
                        credit_amt = v2
                        # Mais seul l'un des deux sera non-nul sur la ligne
                        # → on choisit via heuristique libellé
                        lup = label.upper()
                        is_credit = any(k in lup for k in CREDIT_KW)
                        is_debit  = any(k in lup for k in DEBIT_KW)
                        if is_credit:
                            ...
                            # Garder le plus grand (heuristique)
                            if v2 > v1:
                                debit_amt = None
                            else:
                                credit_amt = None
                    else:
                        # Les deux sont dans la même zone → un seul montant
                        val = v2  # le 2e avant le solde
                        lup = label.upper()
                        is_credit = any(k in lup for k in CREDIT_KW)
                        is_debit  = any(k in lup for k in DEBIT_KW)
                        if is_credit and not is_debit:
                    ...
                    # nb >= 4 : [date_val, débit, crédit, solde]
                    v1, x1 = numeric_blocks[nb-3]
                    v2, x2 = numeric_blocks[nb-2]
                    lup = label.upper()
                    is_credit = any(k in lup for k in CREDIT_KW)
                    is_debit  = any(k in lup for k in DEBIT_KW)
                    if is_credit and not is_debit:
                        credit_amt = v2 if x2 > x1 else v1
                    elif is_debit and not is_credit:
                        debit_amt = v1 if x1 < x2 else v2
                    else:
                        debit_amt  = v1
                        credit_amt = v2
