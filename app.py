...
def extract_iban(text):
    ...
    Stratégies (ordre de priorité) :
      1. Mot-clé IBAN suivi du numéro (tolérance casse, espaces, tirets)
      2. IBAN nu — format international : CC DD + 11-30 alphanum (avec ou sans espaces)
      3. Fallback UEMOA : pays africains sans mot-clé
    """
    # Normaliser les espaces insécables et caractères parasites
    text_clean = text.replace('\xa0', ' ').replace('\u202f', ' ')

    # ── 1. Avec mot-clé IBAN ──────────────────────────────────────────────────
    m = re.search(
        r'(?:IBAN|I\.?B\.?A\.?N\.?)\s*[:\-]?\s*'
        r'([A-Za-z]{2}[\s\-]?\d{2}[\s\-A-Za-z0-9]{10,38})',
        text_clean, re.IGNORECASE
    )
    if m:
        raw = re.sub(r'[\s\-]', '', m.group(1)).upper()
        raw = re.sub(r'[^A-Z0-9]', '', raw)
        # Valider longueur minimale (15 chars) et code pays 2 lettres + 2 chiffres
        if len(raw) >= 15 and re.match(r'^[A-Z]{2}\d{2}', raw):
            return raw[:34]  # max IBAN = 34 chars

    # ── 2. IBAN nu (sans mot-clé) — format groupé par 4 (ex: FR76 3000 4028…) ─
    # Cherche une séquence : 2 lettres + 2 chiffres + blocs alphanum séparés par espaces
    m2 = re.search(
        r'\b([A-Z]{2}\d{2})(?:[\s]?[A-Z0-9]{4}){3,8}\b',
        text_clean.upper()
    )
    if m2:
        # Récupérer le bloc complet depuis le début du match
        start = m2.start()
        chunk = text_clean.upper()[start:start+50]
        # Extraire jusqu'au premier caractère non-IBAN (ni alphanum ni espace)
        raw_chunk = re.match(r'[A-Z0-9\s]+', chunk)
        if raw_chunk:
            raw = re.sub(r'\s+', '', raw_chunk.group(0))
            raw = re.sub(r'[^A-Z0-9]', '', raw)
            if len(raw) >= 15:
                return raw[:34]

    # ── 3. Fallback UEMOA/BCEAO — pays africains sans mot-clé ─────────────────
    ...
    if m3:
        raw = re.sub(r'\s+', '', m3.group(1)).upper()
        raw = re.sub(r'[^A-Z0-9]', '', raw)
        if len(raw) >= 15:
            return raw[:34]
...
def iban_to_rib(iban, info=None):
    ...
    if country in _UEMOA_IBAN and len(c) >= 20:
        ...
        agence      = bban[5:10]
        compte_raw  = bban[10:-2] if len(bban) >= 13 else bban[10:]
...
def _afr_header(pages_text):
    ...
    # ── 1. Extraction directe du RIB ligne "Banque Agence Compte Clé" ────────
    # Ex Coris : "SN213 01001 02341624101 33"
    # Ex SG    : "SN011 01005 005000458982 90"
    # Le séparateur peut être un espace, une tabulation, un tiret ou plusieurs espaces.
    _SEP = r'[\s\t\-]+'
    rib_match = re.search(
    ...
    if rib_match:
        info['_rib_bank']    = rib_match.group(1)   # ex: SN213 ou SN011
        info['_rib_agency']  = rib_match.group(2)   # ex: 01001 ou 01005
        info['_rib_account'] = rib_match.group(3)   # ex: 02341624101
        info['_rib_key']     = rib_match.group(4)   # ex: 33 ou 90
    # Tentative supplémentaire : "Code Banque ... Agence ... Compte ... Clé RIB"
    # sous forme de tableau avec en-têtes (PDF structuré Coris/BSIC/CBAO)
    if not rib_match:
        rib_match2 = re.search(
            r'(?:Code\s+Banque|Banque)[^\n]{0,30?}'
            r'([A-Z]{0,2}\d{3,5})'
            r'.{0,20?}(?:Agence)[^\n]{0,30?}(\d{4,6})'
            r'.{0,20?}(?:Compte)[^\n]{0,30?}(\d{8,14})'
            r'.{0,20?}(?:Cl[eé]\s+RIB)[^\n]{0,30?}(\d{2})\b',
        ...
        if rib_match2:
            info['_rib_bank']    = rib_match2.group(1)
            info['_rib_agency']  = rib_match2.group(2)
            info['_rib_account'] = rib_match2.group(3)
            info['_rib_key']     = rib_match2.group(4)
    # ── Extraction IBAN : déléguer à extract_iban() centralisé ─────────────
    # Gère tous les formats : avec mot-clé, IBAN nu groupé par 4, UEMOA/BCEAO
    if not info['iban']:
        info['iban'] = extract_iban(text)

    # ── 2. Numéro de compte brut (si pas d'IBAN) ─────────────────────────────
    if not info['iban']:
        m4 = re.search(
...
def parse_sg_afrique(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # SG Sénégal : Débit ≈ x0 430–510, Crédit ≈ x0 510–600, Solde ≈ x0 600+
            # La frontière 510 est correcte mais les montants XOF (entiers) peuvent
            # démarrer quelques pixels avant ou après → on tolère ±20px.
            is_credit_kw = any(k in label_up for k in CREDIT_KW)
            is_debit_kw  = any(k in label_up for k in DEBIT_KW)

            if nb == 1:
                val, x0 = resolved[0]
                # Frontière mesurée : débit < 430, crédit ≥ 430
                if x0 >= 430:
                    credit_amt = val
                ...
                val, x0 = resolved[0]
                val2, x0_2 = resolved[1]
                if x0 >= 430:
                    # Premier bloc déjà en zone crédit → crédit seul
                    credit_amt = val
                elif x0_2 >= 430:
                    # Classique : débit à gauche, crédit à droite
                    debit_amt = val
                    credit_amt = val2
                else:
                    # Les deux blocs sont en zone débit → débit + solde (exclure solde)
                    debit_amt = val

            else:
                # nb >= 3 : exclure le solde (bloc le plus à droite, x0 ≥ 520)
                ...
                if len(candidates) == 1:
                    ...
                    if c_right[1] >= 430 and c_left[1] < 430:
                        # Colonnes bien séparées : débit gauche, crédit droite
                        debit_amt  = c_left[0]
                        credit_amt = c_right[0]
                    elif c_right[1] >= 430:
                        # Les deux à droite → crédit
                        credit_amt = c_right[0]
                    else:
                        # Tout à gauche → sémantique
...
def generate_ofx(info, txns, target='quadra', currency='EUR'):
    ...
    # ── ACCTID : Quadra attend l'IBAN complet dans ce champ.
    #    On utilise l'IBAN quand il est disponible, sinon le fragment RIB.
    acctid = iban_full if iban_full else aid
