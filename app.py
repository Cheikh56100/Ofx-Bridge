...
#   Libellé        : x0 ≈ 130-380  (élargi : "PAR :", "FAV :" commencent à x0≈140)
#   Débit (XOF)    : x0 ≈ 370-470
#   Crédit (XOF)   : x0 ≈ 460-560
#   Solde (XOF)    : x0 ≥ 550  (ignoré)
# Particularité : montants XOF entiers sans décimales, fragmentés en plusieurs tokens
#   Les libellés multi-lignes contiennent le bénéficiaire (PAR/FAV) sur la ligne suivante
def parse_bnde(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'LIBELLÉ', 'LIBELLE', 'VALEUR', 'DÉBIT', 'DEBIT',
            ...
            'CRÉDIT (XOF)', 'SOLDE AU', 'CODE CLIENT', 'NUMÉRO DE COMPTE',
            'LIBELLÉ DU COMPTE', 'CODE IBAN', 'NOM DU', 'LIBELLE DU'}
    SKIP_START = ('SOLDE', 'TOTAL', 'A REPORTER', 'PAGE', 'BNDE', 'CODE', '(*)')
    SKIP_EXACT = {'SOLDE (XOF)', 'DÉBIT (XOF)', 'CRÉDIT (XOF)', 'DATE', 'VALEUR',
                  'LIBELLÉ DE L\'OPÉRATION', 'LIBELLE DE L\'OPERATION'}

    full_text = ' '.join(pages_text)
    # Solde final — plusieurs patterns BNDE
    for pat in [
        r'Solde\s+\([A-Z]+\)\s+au\s+[\d/]+\s*:\s*([\d\s]+)',
        r'Solde\s+\(XOF\)\s+au\s+[\d/]+\s*:\s*([\d\s]+)',
        r'Total\s+[\d\s]+\s+([\d\s]+)\s*$',
    ]:
        m_bal = re.search(pat, full_text, re.IGNORECASE | re.MULTILINE)
        if m_bal:
    ...
    # Calibration dynamique des colonnes débit/crédit sur la 1ère page
    debit_x_min, debit_x_max   = 355, 470
    credit_x_min, credit_x_max = 460, 565
    for pw in pages_words[:1]:
        _fd = _fc = False
        for w in sorted(pw, key=lambda x: x['x0']):
            wt = w['text'].upper().strip().replace('É', 'E').replace('é', 'E')
            if not _fd and wt in ('DEBIT',) and w['x0'] > 280:
                debit_x_min = max(280, w['x0'] - 15)
                debit_x_max = w['x0'] + 110
                _fd = True
            elif _fd and not _fc and wt in ('CREDIT',) and w['x0'] > debit_x_min + 30:
                credit_x_min = w['x0'] - 15
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Libellé : x0 ≈ 130–380 (élargi pour capter "PAR :", "FAV :" etc.)
            label_words = [w for w in row
                           if 130 <= w['x0'] < 380
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            label = ' '.join(w['text'] for w in label_words).strip()
            ...
            if not label or len(label) < 2:
                i += 1; continue
            if any(label_up == s for s in SKIP_EXACT):
                i += 1; continue
            if any(s in label_up for s in SKIP):
                # Vérifier que ce n'est pas un libellé partiel contenant un mot-clé
                useful_words = [w for w in label.split() if re.search(r'[A-Za-zÀ-ÿ]{3,}', w)]
            ...
            debit_words  = [w for w in row
                            if debit_x_min <= w['x0'] < debit_x_max
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            credit_words = [w for w in row
                            if credit_x_min <= w['x0'] < credit_x_max
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            debit_amt  = _uba_join_amount(debit_words)
            credit_amt = _uba_join_amount(credit_words)

            # Fallback : si pas de montant détecté, scan large à droite
            if debit_amt is None and credit_amt is None:
                right_w = sorted([w for w in row if w['x0'] >= 340
                                   and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])],
                                  key=lambda x: x['x0'])
                num_blocks, cur, prev_x1 = [], [], None
                for w in right_w:
                    is_num = bool(re.match(r'^\d+$', w['text']))
                    if not is_num:
                        if cur: num_blocks.append(cur); cur = []
                        prev_x1 = None; continue
                    if prev_x1 is not None and (w['x0'] - prev_x1) > 45:
                ...
                resolved = [(v, b[0]['x0']) for b in num_blocks
                            for v in [_uba_join_amount(b)] if v is not None and v >= 100]
                # Exclure le solde (dernier bloc si ≥ 3 blocs)
            ...
            # Mémo lignes suivantes (PAR :, FAV :, bénéficiaire, téléphone)
            memo_parts = []
            j = i + 1
            while j < len(rows):
                r2 = rows[j]
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                               for w in r2):
                    break
                # Capturer aussi les mots à x0 ≈ 80-380 (bénéficiaire)
                nl = ' '.join(w['text'] for w in r2
                              if 80 <= w['x0'] < 380
                              and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])).strip()
                if nl and not any(s == nl.upper() for s in SKIP_EXACT):
                    memo_parts.append(nl)
                j += 1
            i = j
    ...
    if not txns:
        ...
        # Pattern : DATE_OP [DATE_VAL] LIBELLÉ ... DÉBIT CRÉDIT SOLDE
        date_re = re.compile(r'^(\d{2}/\d{2}/\d{4})\s+(?:\d{2}/\d{2}/\d{4}\s+)?(.+)')
        credit_kw = ('VERSEMENT', 'VIREMENT RECU', 'VIREMENT INTER', 'REMISE', 'CREDIT',
                     'DÉBLOCAGE', 'DEBLOCAGE', 'ANNUL', 'RECOUVREMENT', 'SWIFT',
                     'PAR :', 'PAR:', 'TIRE :', 'REMISE DE CHEQUES', 'SORT CHEQUES',
                     'TRF-RECU', 'TRF RECU', 'BENEF RECU', 'RETOUR VRT', 'CNCA')
        debit_kw  = ('RETRAIT', 'CHEQ', 'CHQ', 'AGIOS', 'FRAIS', 'COMMISSION', 'APPEL',
                     'ABONNEMENT', 'ROUTAGE', 'VIREMENT AUTRE', 'FAV :', 'FAV:',
                     'BENEF :', 'BENEF:', 'BENEFICIAIRE', 'PACK', 'PRELEVEMENT',
                     'FORCAGE', 'FRAIS DE ROUTAGE', 'FRAIS ROUTAGE')
        ...
        for line in full_lines:
            ...
            if not m:
                # Ligne de continuation — enrichir le libellé en cours
                if pending_label is not None and re.search(r'[A-Za-zÀ-ÿ]{3,}', line):
                    pending_label += ' ' + line
                continue

            # Sauvegarder la transaction précédente si elle n'a pas pu être résolue
            pending_label = None
            pending_date  = None
            ...
            if any(s in rest_up for s in ('SOLDE', 'TOTAL', 'EXTRAIT', 'AGIOS DU')):
                # Agios = débit (frais)
                if 'AGIOS DU' in rest_up:
                    amounts = re.findall(r'\b(\d{1,3}(?:\s\d{3})*)\b', rest)
                    amounts_p = [parse_amount(a.replace(' ', '')) for a in amounts
                                 if parse_amount(a.replace(' ', '')) and
                                 parse_amount(a.replace(' ', '')) >= 100]
                    if amounts_p:
                        date_ofx = date_full_to_ofx(date_str)
                        txns.append(_make_txn(date_ofx, -amounts_p[0], 'AGIOS', ''))
                continue
            ...
            # Libellé : tout le texte alphanumérique avant les premiers chiffres
            label_m = re.match(r'^([A-Za-zÀ-ÿ\s\-\'\./,&:°0]+(?:\s+[A-Za-zÀ-ÿ\-\'.]+)*)', rest)
            label = label_m.group(1).strip() if label_m else rest[:80].strip()
            # Nettoyer les numéros en fin de libellé
            label = re.sub(r'\s+\d[\d\s]*$', '', label).strip()
            if not label or len(label) < 3:
                continue
            if any(label.upper() == s for s in SKIP_EXACT):
                continue
            ...
            # Les montants sur une même ligne sont en général : DEBIT CREDIT SOLDE
            # Si un seul montant → utiliser mots-clés ; si deux → 1er=débit/crédit, 2e=autre
            if len(amounts_parsed) >= 3:
                # débit, crédit, solde → prendre les 2 premiers
                d_val, c_val = amounts_parsed[0], amounts_parsed[1]
                # Si débit > 0, enregistrer débit ; sinon crédit
                if d_val > 0 and is_debit and not is_credit:
                    txns.append(_make_txn(date_ofx, -d_val, name, memo_out))
                elif c_val > 0 and is_credit and not is_debit:
                    txns.append(_make_txn(date_ofx, c_val, name, memo_out))
                else:
                    # Ambiguïté : utiliser le premier montant non-solde
                    amt = amounts_parsed[0]
                    signed = amt if is_credit else -amt
                    txns.append(_make_txn(date_ofx, signed, name, memo_out))
            else:
                amt = amounts_parsed[0]
...
def parse_sg_afrique(pages_words, pages_text, _pdf_path=''):
    ...
    # ── Calibration dynamique des colonnes depuis l'en-tête ─────────────────
    # On cherche les mots "Débit"/"Crédit"/"Solde" dans les premières lignes
    # pour déterminer précisément les frontières de colonnes du PDF.
    # Valeurs par défaut SGBS Sénégal jan-2026 (relevé réel) :
    _sg_debit_x_max  = 430   # tout ce qui est < 430 → Débit
    _sg_credit_x_min = 430   # tout ce qui est ≥ 430 et < 510 → Crédit
    _sg_solde_x_min  = 510   # tout ce qui est ≥ 510 → Solde (ignoré)

    for pw in pages_words[:2]:          # scanner les 2 premières pages
        for w in pw:
            wt = w['text'].strip().upper()
            # "Débit" apparaît centré sur sa colonne — on prend son bord droit + marge
            if wt in ('DÉBIT', 'DEBIT') and 300 < w['x0'] < 500:
                _sg_debit_x_max  = w['x1'] + 20
                _sg_credit_x_min = w['x1'] + 20
            # "Crédit" — on en déduit la frontière crédit/solde
            elif wt in ('CRÉDIT', 'CREDIT') and 350 < w['x0'] < 560:
                _sg_solde_x_min  = w['x1'] + 20
        if _sg_debit_x_max != 430:      # calibration réussie
            break
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Débit/Crédit/Solde : calibrés dynamiquement depuis l'en-tête
            ...
            # SG Sénégal / SG Afrique — positions calibrées dynamiquement
            # depuis l'en-tête du relevé (mots "Débit" et "Crédit").
            # Valeurs par défaut issues du relevé SGBS Sénégal réel (jan-2026) :
            #   Débit  : x0 ≈ 358–430
            #   Crédit : x0 ≈ 437–510
            #   Solde  : x0 ≈ 507+     (ignoré)
            DEBIT_X_MAX  = _sg_debit_x_max
            CREDIT_X_MIN = _sg_credit_x_min
            SOLDE_X_MIN  = _sg_solde_x_min
...
def parse_cbao(pages_words, pages_text, _pdf_path=''):
    ...
    # Valeurs par défaut CBAO Sénégal (relevé réel mars-2026) :
    #   Débit  : x0 ≈ 342–420  → debit_x_max  = 420
    #   Crédit : x0 ≈ 421–500  → credit_x_min = 420
    #   Solde  : x0 ≈ 503+
    debit_x_min  = 320
    debit_x_max  = 420
    credit_x_min = 420
    ...
    # pdfplumber fragmente souvent "Débit (XOF)" en deux tokens séparés :
    # "Débit" et "(XOF)". On cherche donc le mot seul, puis on affine.
    _found_debit = _found_credit = False
    for pw in pages_words[:1]:
        for w in pw:
            wt_up = w['text'].strip().upper()
            # Normalise é/É → E pour matcher indépendamment de l'encodage du PDF
            wt_norm = wt_up.replace('é', 'E').replace('É', 'E')
            if not _found_debit and wt_norm in ('DEBIT', 'DEBIT') and w['x0'] > 200:
                debit_x_min  = max(200, w['x0'] - 20)
                debit_x_max  = w['x1'] + 80
                _found_debit = True
            elif _found_debit and not _found_credit and wt_norm in ('CREDIT', 'CREDIT') and w['x0'] > debit_x_max - 50:
                credit_x_min = w['x0'] - 10
                credit_x_max = w['x1'] + 80
                _found_credit = True
...
#   Date op     : x0 ≈ 41–75  (DD/MM/YY ou DD/MM/YYYY)
#   Description : x0 ≈ 88–255
#   Référence   : x0 ≈ 255–310 (ignoré)
#   Date valeur : x0 ≈ 310–370 (ignoré)
#   Débit       : x0 ≈ 360–445  (montants négatifs = débit, rarement positif)
#   Crédit      : x0 ≈ 440–515
#   Solde       : x0 ≥ 515 (ignoré)
# Particularité : les montants Débit sont affichés négatifs (ex: -200 000,00)
#   et les Crédits positifs (ex: 600 200,00). La colonne "Solde courant" est à droite.
# Calibration dynamique : le parseur lit les en-têtes "Débit" et "Crédit" sur
#   la 1re page pour ajuster automatiquement les zones de colonnes.
# ════════════════════════════════════════════════════════════════════════════
def parse_boa(pages_words, pages_text, _pdf_path=''):
    ...
    def _boa_parse_amount_raw(words):
        """Parse un montant BOA fragmenté en plusieurs tokens.
        Gère les cas : '-200 000,00' (débit), '600 200,00' (crédit), '-1 000 000,00' (débit).
        Les tokens sont joints intelligemment : tous sauf le dernier forment l'entier,
        le dernier (contenant ',') fournit la décimale."""
        if not words:
            return None
        words_sorted = sorted(words, key=lambda w: w['x0'])
        full = ' '.join(w['text'] for w in words_sorted).replace('\xa0', ' ').strip()
        neg = full.startswith('-')
        full = full.lstrip('-').strip()
        tokens = full.split()
        # Cas montant fragmenté : dernier token contient une virgule
        if len(tokens) >= 2 and ',' in tokens[-1]:
            integer_part = ''.join(tokens[:-1])   # ex: "200", "1000", "600"
            decimal_part = tokens[-1]              # ex: "000,00", "200,00"
            joined = integer_part + decimal_part   # ex: "200000,00", "1000000,00", "600200,00"
            v = parse_amount(joined)
            if v is not None and v > 0:
                return -v if neg else v
        # Cas montant simple (un seul token)
        v = parse_amount(full.replace(' ', ''))
        if v is not None and v > 0:
            return -v if neg else v
        return None

    def _boa_detect_amounts(row):
        """Détecte débit/crédit BOA en gérant le cas où un montant XOF fragmenté
        chevauche les colonnes débit et crédit.
        Logique : si le premier fragment (zone débit) est un entier positif sans signe
        et que la zone crédit contient la suite (ex: '200,00'), on fusionne en crédit.
        Si la zone débit contient un signe négatif, c'est un débit pur."""
        dw = [w for w in row if debit_x_min  <= w['x0'] < credit_x_min]
        cw = [w for w in row if credit_x_min <= w['x0'] < credit_x_max]

        dw_text = ' '.join(w['text'] for w in sorted(dw, key=lambda w: w['x0'])).strip()
        is_negative = dw_text.startswith('-')

        if not is_negative and dw and cw:
            # Possible montant de crédit fragmenté entre les deux zones : on fusionne
            merged = _boa_parse_amount_raw(dw + cw)
            if merged is not None and merged > 0:
                return None, merged   # (debit=None, credit=montant)
            # Pas de fusion possible : traiter séparément
            return _boa_parse_amount_raw(dw), _boa_parse_amount_raw(cw)

        # Débit négatif ou zone crédit seule
        return _boa_parse_amount_raw(dw), _boa_parse_amount_raw(cw)

    # Calibration dynamique des colonnes sur la première page
    # Valeurs par défaut mesurées sur relevé BOA Sénégal (Villa Yemaya jan. 2026)
    # Les zones sont non-chevauchantes : [debit_x_min – credit_x_min) et [credit_x_min – credit_x_max)
    debit_x_min  = 360
    credit_x_min = 453   # séparateur débit/crédit = x0 de l'en-tête "Crédit"
    credit_x_max = 515   # début de la colonne Solde courant
    ...
    for pw in pages_words[:1]:
        for w in pw:
            ...
            if wt in ('DÉBIT', 'DEBIT') and w['x0'] > 200:
                debit_x_min = max(200, w['x0'] - 20)
            elif wt in ('CRÉDIT', 'CREDIT') and w['x0'] > debit_x_min + 30:
                credit_x_min = w['x0']   # séparateur exact
                credit_x_max = w['x0'] + 70
            elif wt in ('SOLDE', 'SOLDE COURANT') and w['x0'] > credit_x_min + 20:
                credit_x_max = min(credit_x_max, w['x0'] - 2)
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            row = rows[i]
            # Date op : x0 < 80, format DD/MM/YY ou DD/MM/YYYY
            date_words = [w for w in row if w['x0'] < 80
            ...
            # Description : x0 ≈ 88–255 (avant la colonne Référence)
            label_words = [w for w in row if 88 <= w['x0'] < 255]
            label = ' '.join(w['text'] for w in label_words).strip()
            label_up = label.upper()
            if not label or len(label) < 3:
                i += 1; continue
            ...
            # Montants débit/crédit avec gestion du chevauchement de colonnes
            debit_raw, credit_raw = _boa_detect_amounts(row)
            ...
            while j < len(rows):
                r2 = rows[j]
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{2,4}$', w['text'])
                               for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2 if 88 <= w['x0'] < 255).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j
...
def parse_orabank(pages_words, pages_text, _pdf_path=''):
    ...
    SKIP = {'TOTAL', 'SOLDE', 'DATE', 'VALEUR', 'LIBELLÉ', 'LIBELLE', 'DÉBIT', 'DEBIT',
            ...
            'SOLDE AU', 'VEUILLEZ NOTER', 'TOTAL GENERAL DES MOUVEMENTS',
            'RELEVE D\'IDENTITE BANCAIRE', 'DOMICILIATION', 'INTITULE COMPTE',
            'EXTRAIT DE COMPTE'}
    SKIP_START = ('SOLDE', 'TOTAL', 'ANCIEN SOLDE', 'NOUVEAU SOLDE', 'VEUILLEZ',
                  'NCART:', 'COMMERCANT', 'CODE AUTH', 'TERMINAL :',
                  'RETRAIT GAB', 'RETRAIT DAB VISA')
    # Libellés de lignes de détail à ignorer (informations VISA)
    SKIP_DETAIL = ('NCART:', 'COMMERCANT :', 'COMMERCANT:', 'CODE AUTH :', 'CODE AUTH:',
                   'TERMINAL :', 'RETRAIT GAB VISA', 'RETRAIT DAB VISA')

    full_text = ' '.join(pages_text)
    # Solde final — plusieurs patterns Orabank
    for pat in [
        r'Solde\s+au\s*[:\-]?\s*\d{2}/\d{2}/\d{4}\s+([\d\s]+)',
    ...
    # Orabank AHC : Date | Libellé opération | Valeur | Débit | Crédit | Solde
    # Positions AHC : Débit≈410, Crédit≈490, Solde≈550
    debit_x_min  = 380
    debit_x_max  = 490
    credit_x_min = 480
    credit_x_max = 580

    for pw in pages_words[:2]:  # chercher sur les 2 premières pages
        _found_d = _found_c = False
        for w in sorted(pw, key=lambda x: x['x0']):
            wt = w['text'].upper().strip().replace('É','E').replace('é','e')
            if not _found_d and wt in ('DEBIT',) and w['x0'] > 280:
                debit_x_min = max(280, w['x0'] - 20)
                debit_x_max = w['x0'] + 110
                _found_d = True
            elif _found_d and not _found_c and wt in ('CREDIT',) and w['x0'] > debit_x_min + 30:
                credit_x_min = w['x0'] - 20
                credit_x_max = w['x0'] + 110
                _found_c = True
                break
        if _found_d and _found_c:
            break
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Libellé : x0 ≈ 75–400 (zone élargie pour capter les libellés Orabank)
            label_words = [w for w in row
                           if 75 <= w['x0'] < 400
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])]
            label = ' '.join(w['text'] for w in label_words).strip()
            ...
            # Mémo lignes suivantes — ignorer les détails VISA techniques
            memo_parts = []
            j = i + 1
            while j < len(rows):
                r2 = rows[j]
                ...
                if nl:
                    nl_up = nl.upper()
                    skip_this = any(nl_up.startswith(d) for d in SKIP_DETAIL)
                    skip_this = skip_this or any(s == nl_up for s in SKIP)
                    if not skip_this:
                        memo_parts.append(nl)
                j += 1
            i = j
...
# Positions mesurées sur relevés NSIA AAG SENEGAL (mars 2026) :
#   Date Transact : x0 ≈ 34–75   (DD/MM/YYYY)
#   Détails       : x0 ≈ 80–250  (description — pdfplumber coupe souvent les lignes)
#   Cheque N°     : x0 ≈ 200–280 (ignoré)
#   Agence        : x0 ≈ 285–325 (3 chiffres, ignoré)
#   Date Valeur   : x0 ≈ 325–370 (ignoré)
#   Mouv. Débit   : x0 ≈ 385–430 (en-têtes "Mouv."@382 + "Débit"@406, 2 tokens séparés)
#   Mouv. Crédit  : x0 ≈ 430–528 (en-têtes "Mouv."@451 + "Crédit"@475, 2 tokens séparés)
#   Solde         : x0 ≥ 528     (ignoré)
# Particularité : les montants XOF sont fragmentés (ex: "750"@398 + "000"@412)
#   et il n'y a PAS de signe négatif — le type débit/crédit est donné par la colonne.
#   La calibration lit séparément "Mouv." (ignoré) puis "Débit"/"Crédit".
# Particularité IROKO BEACH : virements TPE (SV CLEARING) génèrent 3 lignes
#   (PRIN, COMM, TAX). On ne garde que PRIN.
# ════════════════════════════════════════════════════════════════════════════
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    full_text = ' '.join(pages_text)
    # Solde final depuis "SOLDE  1 136 108" en fin de relevé
    for pat in [
        r'SOLDE\s+([\d][\d\s]{3,})\s*$',
    ...
    # ── Calibration dynamique sur les en-têtes de la première page ───────────
    # "Mouv." et "Débit"/"Crédit" sont deux tokens séparés par pdfplumber.
    # On repère le token "Débit" (x0≈406) et "Crédit" (x0≈475) dans la zone >300.
    # Zones non-chevauchantes : [debit_x_min – credit_x_min) et [credit_x_min – solde_x)
    debit_x_min  = 380   # début plausible de la colonne Mouv. Débit
    credit_x_min = 450   # séparateur débit/crédit = x0 de l'en-tête "Crédit"
    solde_x      = 525   # début de la colonne Solde (exclue)

    for pw in pages_words[:1]:
        found_debit  = False
        found_credit = False
        for w in sorted(pw, key=lambda w: w['x0']):
            wt = w['text'].upper().strip()
            # "Débit" seul (après "Mouv.") ou "MOUV. DÉBIT" concaténé
            if not found_debit and wt in ('DÉBIT', 'DEBIT') and w['x0'] > 300:
                debit_x_min  = max(300, w['x0'] - 25)
                found_debit  = True
            # "Crédit" seul (après "Mouv.") ou "MOUV. CRÉDIT" concaténé
            elif found_debit and not found_credit and wt in ('CRÉDIT', 'CREDIT') and w['x0'] > debit_x_min + 20:
                credit_x_min = w['x0']
                found_credit = True
            # "Solde" — tronque la zone crédit
            elif found_credit and wt == 'SOLDE' and w['x0'] > credit_x_min + 20:
                solde_x = w['x0'] - 2
                break

    def _nsia_join_amount(words):
        """Joint les fragments d'un montant NSIA XOF (entiers sans virgule).
        Ex : ['750', '000'] → 750000 ou ['2', '100', '000'] → 2100000.
        Les montants NSIA n'ont PAS de signe négatif."""
        if not words:
            return None
        ws = sorted(words, key=lambda w: w['x0'])
        # Vérifier que tous les tokens sont numériques
        texts = [w['text'] for w in ws]
        if not all(re.match(r'^\d+$', t) for t in texts):
            # Tenter quand même un parse simple
            full = ''.join(texts)
            try:
                return float(full) if full else None
            except ValueError:
                return None
        # Concaténer et convertir
        joined = ''.join(texts)
        try:
            return float(joined)
        except ValueError:
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Détails Transaction : x0 ≈ 80–245
            # Exclure les dates valeur, numéros d'agence et numéros de chèque
            label_words = [w for w in row if 80 <= w['x0'] < 245
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                           and not re.match(r'^\d{3,7}$', w['text'])]
            label = ' '.join(w['text'] for w in label_words).strip()

            # NSIA multi-ligne inversé : le libellé complet est parfois réparti sur
            # les lignes précédant la ligne qui porte la date + montant.
            # Si la ligne de date n'a pas de libellé, on remonte pour le récupérer.
            if (not label or len(label) < 3) and i > 0:
                back_parts = []
                k = i - 1
                while k >= 0:
                    rk = rows[k]
                    if any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text']) for w in rk):
                        break
                    nl = ' '.join(w['text'] for w in rk
                                  if 80 <= w['x0'] < 245
                                  and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                                  and not re.match(r'^\d{3,7}$', w['text'])).strip()
                    if nl:
                        back_parts.insert(0, nl)
                    k -= 1
                label = ' '.join(back_parts).strip()
            ...
            # Ignorer les lignes COMM et TAX des virements SV CLEARING (TPE)
            # On ne garde que les lignes :-PURCHASE (crédit net au commerçant) ou PRIN
            is_sv_clearing = 'SV CLEARING' in label_up
            if is_sv_clearing:
                # Garder uniquement les lignes :-PURCHASE (montant net crédit) ou PRIN
                # Ignorer COMM (commission) et TAX (taxe)
                is_comm_or_tax = (
                    label_up.endswith(':COMM') or label_up.endswith(':TAX') or
                    label_up.endswith(' COMM') or label_up.endswith(' TAX') or
                    ':COMM' in label_up or ':TAX' in label_up or
                    re.search(r'\bCOMM\b', label_up) or re.search(r'\bTAX\b', label_up)
                )
                is_purchase_principal = (
                    ':-PURCHASE' in label_up or ':PURCHASE' in label_up or
                    'PRIN' in label_up
                )
                # Si c'est un COMM ou TAX (pas un montant principal), ignorer
                if is_comm_or_tax and not is_purchase_principal:
                    i += 1; continue

            # ── Mouv. Débit  : [debit_x_min – credit_x_min) ─────────────────
            # ── Mouv. Crédit : [credit_x_min – solde_x)      ─────────────────
            debit_words  = [w for w in row
                            if debit_x_min <= w['x0'] < credit_x_min
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                            and re.match(r'^\d+$', w['text'])]
            credit_words = [w for w in row
                            if credit_x_min <= w['x0'] < solde_x
                            and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                            and re.match(r'^\d+$', w['text'])]

            debit_amt  = _nsia_join_amount(debit_words)
            credit_amt = _nsia_join_amount(credit_words)

            # Pour les SV CLEARING :-PURCHASE, le montant est toujours un crédit
            # même si pdfplumber le place dans la colonne débit
            if is_sv_clearing and ':-PURCHASE' in label_up:
                if debit_amt and not credit_amt:
                    credit_amt = debit_amt
                    debit_amt  = None

            # Mémo lignes suivantes (libellé multi-ligne)
            memo_parts = []
            j = i + 1
            while j < len(rows):
                r2 = rows[j]
...
def parse_ecobank(pages_words, pages_text, _pdf_path=''):
    ...
    if is_esn_format:
        ...
        #   Payments (débit) : x0 ≈ 490–630
        #   Deposits (crédit): x0 ≈ 630–760
        #   Balance          : x0 ≥ 760

        # Calibration dynamique : chercher les en-têtes "Payments" et "Deposits"
        pay_x_min, pay_x_max = 480, 640
        dep_x_min, dep_x_max = 630, 780
        for pw in pages_words[:2]:
            for w in pw:
                wt = w['text'].upper().strip()
                if wt == 'PAYMENTS' and w['x0'] > 350:
                    pay_x_min = max(350, w['x0'] - 20)
                    pay_x_max = w['x0'] + 160
                elif wt == 'DEPOSITS' and w['x0'] > pay_x_min + 50:
                    dep_x_min = w['x0'] - 20
                    dep_x_max = w['x0'] + 160
        ...
        for pw in pages_words:
            ...
            while i < len(rows):
                ...
                # Montants sur la même ligne que la date — utiliser zones calibrées
                pay_words = [w for w in row if pay_x_min <= w['x0'] < pay_x_max]
                dep_words = [w for w in row if dep_x_min <= w['x0'] < dep_x_max]
                ...
                # Si aucun montant avec zones calibrées, essai avec tous les tokens à droite
                if debit_amt is None and credit_amt is None:
                    all_right = sorted([w for w in row if w['x0'] >= 450], key=lambda x: x['x0'])
                    # Grouper les blocs de montants
                    groups, cur, prev_x1 = [], [], None
                    for w in all_right:
                        is_num = bool(re.search(r'[\d,\.]', w['text']) and
                                      re.match(r'^[\dXOF,\.\s]+$', w['text'], re.IGNORECASE))
                        if not is_num:
                            if cur: groups.append(cur); cur = []
                            prev_x1 = None; continue
                        if prev_x1 is not None and (w['x0'] - prev_x1) > 50:
                    ...
                    for g in groups:
                        ...
                        v = _eco_parse_xof_amount(txt)
                        if v is not None:
                            resolved.append((v, g[0]['x0']))
                    # Balance = dernier bloc, Payment avant-dernier, Deposit avant-avant-dernier
                    if len(resolved) >= 3:
                        # ordre: ..., payment, deposit, balance
                        debit_amt  = resolved[-3][0]
                        credit_amt = resolved[-2][0]
                    elif len(resolved) == 2:
                        debit_amt  = resolved[-2][0]
                        credit_amt = resolved[-1][0]
                    elif len(resolved) == 1:
                        # Pas de contexte suffisant pour distinguer
                        pass
