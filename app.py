...
# ── BNDE : format tableau Date | Valeur | Libellé | Débit (XOF) | Crédit (XOF) | Solde (XOF)
# PDF natif texte — positions mesurées sur PDF BNDE Thiès (jan 2026) :
#   Date opération : x0 ≈ 32–76   (DD/MM/YYYY)
#   Valeur         : x0 ≈ 80–125  (DD/MM/YYYY — ignoré)
#   Libellé        : x0 ≈ 128–340
#   Débit (XOF)    : x0 ≈ 340–430  (en-tête "Débit" à x0=342.1)
#   Crédit (XOF)   : x0 ≈ 430–505  (en-tête "Crédit" à x0=421.4 → montants à x0=446–482)
#   Solde (XOF)    : x0 ≥ 505      (en-tête "Solde" à x0=503.2 — ignoré)
# Particularité : montants XOF entiers sans décimales, fragmentés en tokens (ex: '1' '400' '000')
# Particularité : certaines lignes n'ont PAS de libellé sur la même ligne (libellé sur
#   la ligne suivante, ou partagé avec la ligne précédente) — on cherche en avant et en arrière.
def parse_bnde(pages_words, pages_text, _pdf_path=''):
    ...
    # ── Calibration dynamique sur la première page ────────────────────────────
    # En-têtes : "Débit" à x0≈342, "Crédit" à x0≈421, "Solde" à x0≈503
    # On cherche "Débit" puis "Crédit" dans l'ordre de gauche à droite (x0 croissant),
    # et "Solde" LE PLUS À DROITE pour ne pas confondre avec "Solde" dans les libellés.
    debit_x_min  = 340
    credit_x_min = 430
    solde_x      = 503
    ...
    for pw in pages_words[:1]:
        ...
        for w in sorted(pw, key=lambda w: w['x0']):
            ...
            if not found_d and wt in ('DÉBIT', 'DEBIT') and w['x0'] > 200:
                debit_x_min = max(200, w['x0'] - 25)
                found_d = True
            elif found_d and not found_c and wt in ('CRÉDIT', 'CREDIT') and w['x0'] > debit_x_min + 20:
                credit_x_min = w['x0']  # séparateur exact, PAS de marge négative
                found_c = True
            elif found_c and wt == 'SOLDE' and w['x0'] > credit_x_min + 20:
                solde_candidates.append(w['x0'])
        if solde_candidates:
            solde_x = max(solde_candidates) - 2

    def _bnde_join_amount(words):
        """Joint les tokens entiers XOF BNDE (pas de décimales).
        Ex : ['1','400','000'] → 1400000 ; ['27','325'] → 27325 ; ['100'] → 100.
        Filtre les dates valeur et les soldes (tokens avec signe '-')."""
        if not words:
            return None
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Date opération : DD/MM/YYYY, x0 < 80
            ...
            # On lit les montants AVANT le libellé pour ne pas rater les lignes
            # sans libellé sur la même ligne (PAR :, FAV : souvent sur ligne suivante)
            ...
            credit_words = [w for w in row
                            ...
                            and re.match(r'^\d+$', w['text'])]
            debit_amt  = _bnde_join_amount(debit_words)
            credit_amt = _bnde_join_amount(credit_words)

            # Skip si aucun montant trouvé (lignes d'en-tête, totaux, etc.)
            if debit_amt is None and credit_amt is None:
                i += 1; continue

            # ── Libellé : x0 ≈ 128–credit_x_min ────────────────────────────
            label_words = [w for w in row
                           if 128 <= w['x0'] < credit_x_min
                           and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                           and not re.match(r'^\d+$', w['text'])]
            label = ' '.join(w['text'] for w in label_words).strip()

            # Mémo sur les lignes suivantes (libellé multi-ligne normal)
            memo_parts = []
            j = i + 1
            while j < len(rows):
                r2 = rows[j]
                if any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text']) for w in r2):
                    break
                nl = ' '.join(w['text'] for w in r2
                              if 128 <= w['x0'] < credit_x_min
                              and not re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                              and not re.match(r'^\d+$', w['text'])).strip()
                if nl and not any(s in nl.upper() for s in SKIP):
                    memo_parts.append(nl)
                j += 1
            i = j

            # Si toujours pas de libellé, on remonte sur les lignes précédentes
            # (cas BNDE : "PAR : NOM PRENOM\nADRESSE TEL" puis ligne date+montant)
            if not label and not memo_parts:
                back_parts = []
                k = i - 2  # la ligne juste avant la date courante
                while k >= 0:
                    rk = rows[k]
                    if any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text'])
                           for w in rk):
                        break
