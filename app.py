...
def _sg_amount_in_zone(row, x_min, x_max):
    ...
    if not col: return None
    # Utiliser _uba_join_amount pour gerer les montants fragmentes (ex: "5 850" → 5850)
    v = _uba_join_amount(col)
    if v is not None:
        return v
    return parse_amount(col[-1]['text'])
...
def parse_bsic(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        for row in rows:
            ...
            # Débit (x0 330-440) / Crédit (x0 440-520) / Solde (x0 520+)
            # Positions mesurées sur PDF réel : débit ≈ 361-405, crédit ≈ 446-480, solde ≈ 523+
            debit_words  = [w for w in row if 330 <= w['x0'] < 440]
            credit_words = [w for w in row if 440 <= w['x0'] < 520]
...
# Colonnes : Date+Valeur (concaténées) | No Trs | Description | Mnt Cr | Mnt Débit | Solde
# Positions mesurées sur PDF réel :
#   Date+Valeur : x0 ≈ 19-103 (format "dd/mm/yyyydd/mm/yyyy" concaténé)
#   No Trs      : x0 ≈ 111-152
...
def parse_bis(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        for row in rows:
            ...
            if not label:
                continue
            # Ignorer lignes purement numériques (No de chèque seul)
            ...
            # '0' = colonne vide dans le format iMAL*CSM
...
# Structure réelle mesurée sur PDF :
#   - Le libellé (VISA-PUR, MAINLEVEE...) est sur les lignes PRÉCÉDANT la ligne de date
#   - La ligne de date contient : date (x0<90), date valeur (x0≈269), débit (x0≈330-430),
#     crédit (x0≈430-520), solde (x0≈520+)
#   - Les lignes de suite (No instruction, suite libellé) sont entre les lignes de date
#
# Stratégie : on collecte toutes les lignes par page, on identifie les "date rows",
# puis pour chaque date row, le libellé = texte des lignes situées entre la date row
# précédente (exclue) et la date row courante (exclue).
def parse_uba(pages_words, pages_text, _pdf_path=''):
    info = _afr_header(pages_text)
    txns = []
    SKIP_WORDS = {'TOTAL','SOLDE','DATE','VALEUR','INSTR','OPÉRATION','OPERATION',
                  'DÉBIT','DEBIT','CRÉDIT','CREDIT','UBA','UNITED','BANK','AFRICA',
                  'EXTRAIT','RELEVÉ','TITULAIRE','RIB','BIC','AGENCE','COMPTE',
                  'POUR','LA','PÉRIODE','DU','AU','RELEVÉ'}
    ...
    def _group_num_tokens(words):
        """Regroupe tokens numériques contigus en blocs (un bloc = un montant)."""
    ...
    for pw in pages_words:
        ...
        for i, date_idx in enumerate(date_row_indices):
            ...
            # Libellé = texte des lignes entre la date row précédente et celle-ci
            prev_date_idx = date_row_indices[i - 1] if i > 0 else -1
            label_parts = []
            # Mots-clés d'entête à ignorer ligne par ligne
            ...
            for j in range(prev_date_idx + 1, date_idx):
                r = rows[j]
                # Prendre les mots dans la zone libellé (x0 ~120-400)
                lw = [w for w in r if 115 <= w['x0'] < 410]
                text = ' '.join(w['text'] for w in lw).strip()
                # Ignorer lignes purement numériques (No instruction)
                if not text or re.match(r'^[\d/,\.\s]+$', text):
                    continue
                # Ignorer lignes d'entête de tableau
                text_up = text.upper()
                if any(s in text_up for s in HEADER_SKIP):
                    continue
                # Ignorer lignes contenant des dates seules (période du relevé)
                if re.match(r'^[au\s\d/]+$', text, re.IGNORECASE):
                    continue
                label_parts.append(text)
            label = ' '.join(label_parts).strip()

            # Si pas de libellé dans les lignes précédentes, essayer la ligne de date elle-même
            if not label:
            ...
            if not label:
                continue
            # Vérifier que le libellé n'est pas que des mots d'entête résiduels
            label_up = label.upper()
            # Ignorer uniquement les libellés qui ne contiennent aucun mot "utile"
            has_useful = bool(re.search(r'[A-Za-zÀ-ÿ]{3,}', label) and
                              not all(any(s == w for s in SKIP_WORDS)
                                      for w in label_up.split()))
            if not has_useful:
                continue
            ...
            # Déduction débit/crédit par POSITION x mesurée :
            #   Débit  : x0 ≈ 330-430
            #   Crédit : x0 ≈ 430-520
            #   Solde  : x0 ≈ 520+
            ...
            # Fallback mots-clés si la position ne suffit pas
...
def parse_sg_afrique(pages_words, pages_text, _pdf_path=''):
    ...
    CREDIT_KW = {'VIREMENT','VERSEMENT','REMISE','CREDIT','RECU','SWIFT',
                 'DECAISSEMENT','MAINLEVEE','DEBLOCAGE'}
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            label = ' '.join(w['text'] for w in row if 75 <= w['x0'] < 430).strip()
            label_up = label.upper()
            if not label or any(s in label_up for s in SKIP):
                i += 1; continue

            # --- Détection robuste des montants ---
            # Tous les tokens numériques à droite du libellé (x0 >= 400)
            right_words = sorted([w for w in row if w['x0'] >= 400],
                                 key=lambda w: w['x0'])
            ...
            if nb == 1:
                # Un seul montant : heuristique libellé
                val, x0 = resolved[0]
                is_credit = any(k in label_up for k in CREDIT_KW)
                is_debit  = any(k in label_up for k in DEBIT_KW)
                if is_credit and not is_debit:
                    ...
                    # Position x : débit ~400-510, crédit ~510+
                    if x0 > 500:
                        credit_amt = val
                    else:
                        debit_amt = val
                ...
                # [débit_ou_crédit, solde] → le 1er est le mouvement
                val, x0 = resolved[0]
                _, x1_solde = resolved[1]
                is_credit = any(k in label_up for k in CREDIT_KW)
                is_debit  = any(k in label_up for k in DEBIT_KW)
                if is_credit and not is_debit:
                    credit_amt = val
                    ...
                    # Position : si le montant est dans la zone crédit (droite)
                    if x0 > 490:
                        credit_amt = val
                    else:
                        debit_amt = val
                ...
                # nb >= 3 : [débit?, crédit?, solde] ou [débit, solde] ou [crédit, solde]
                # Les deux derniers blocs avant le solde sont débit et crédit
                # Le solde est le dernier → on l'ignore
                # On cherche parmi resolved[:-1] les montants débit/crédit
                candidates = resolved[:-1]  # exclure le solde
                if len(candidates) == 1:
                    val, x0 = candidates[0]
                    if x0 > 490:
                        credit_amt = val
                    ...
                    # 2 candidats : le plus à gauche = débit, le plus à droite = crédit
                    # Mais sur la même ligne, seul l'un est non-nul (l'autre serait 0 ou absent)
                    # On prend le plus grand des deux comme montant réel
                    # et on déduit le sens par position
                    c_left  = min(candidates, key=lambda x: x[1])
                    c_right = max(candidates, key=lambda x: x[1])
                    # Si les deux ont des valeurs significatives et différentes,
                    # c'est débit à gauche ET crédit à droite
                    if c_right[1] > c_left[1] + 40:
                        # Zones bien séparées : débit gauche, crédit droite
                        is_credit = any(k in label_up for k in CREDIT_KW)
                        is_debit  = any(k in label_up for k in DEBIT_KW)
                        if is_credit and not is_debit:
                        ...
                        # Zones proches : un seul montant réel
                        val = max(c[0] for c in candidates)
                        is_credit = any(k in label_up for k in CREDIT_KW)
                        is_debit  = any(k in label_up for k in DEBIT_KW)
                        if is_credit and not is_debit:
                            credit_amt = val
            ...
            while j < len(rows):
                ...
                if r2 and r2[0]['x0'] < 75 and re.match(r'^\d{2}/\d{2}/\d{4}$', r2[0]['text']):
                    break
                nl = ' '.join(w['text'] for w in r2 if 75 <= w['x0'] < 430).strip()
