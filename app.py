...
def extract_iban(text):
    # IBAN standard : 2 lettres + 2 chiffres + jusqu'à 30 caractères alphanumériques
    m = re.search(r'IBAN\s*:?\s*([A-Z]{2}\d{2}[A-Z0-9][\s A-Z0-9]{8,35})', text)
    if m:
        return re.sub(r'\s+', '', m.group(1)).strip()
    # IBAN UEMOA sans mot-clé (ex : SN08 SN011 01005 …)
    m2 = re.search(r'\b(SN\d{2}[A-Z0-9]{2,4}[\s\d]{15,35})\b', text)
    if m2:
...
def _afr_header(pages_text):
    ...
    # Format : SN08 SN011 01005 005000458982 90  (28 car. sans espaces)
    # On cherche d'abord avec mot-clé
    m = re.search(
        r'(?:IBAN|Code\s+IBAN|Code\s+Iban)\s*[:\-]?\s*'
        r'([A-Z]{2}\d{2}\s?[A-Z0-9]{2,5}\s?[\d\s]{15,30})',
        text, re.IGNORECASE
    )
    if m:
        raw = re.sub(r'\s+', '', m.group(1)).upper()
        # Ne garder que les caractères alphanumériques valides pour un IBAN
        raw = re.sub(r'[^A-Z0-9]', '', raw)[:28]
        if len(raw) >= 15:
            info['iban'] = raw

    # Fallback : IBAN sans mot-clé — pattern SN08 SN011 ...
    if not info['iban']:
        m2 = re.search(
            r'\b([A-Z]{2}\d{2})\s+([A-Z]{2}\d{3})\s+(\d{5})\s+([\d\s]{11,15})\s*(\d{2})\b',
            text
        )
        if m2:
            raw = ''.join(m2.groups()).replace(' ', '')
            info['iban'] = re.sub(r'[^A-Z0-9]', '', raw)[:28]

    # Fallback 2 : juste le code pays + chiffres contigus
    if not info['iban']:
        m3 = re.search(r'\b(SN\d{2}[A-Z0-9\s]{18,30})\b', text)
        if m3:
...
def generate_ofx(info, txns, target='quadra', currency='EUR'):
    bid, brid, aid = iban_to_rib(info.get('iban',''), info=info)
