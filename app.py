...
def parse_amount(s):
    # Supprime tous les types d'espaces blancs (\n, \r, \t, \xa0, espaces normaux)
    s = re.sub(r'\s+', '', str(s)).replace('\xa0','').replace('*','').strip()
...
def parse_bsic(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        rows = group_words_by_row(pw, tol=2.0)  # Tolérance réduite : tableaux BSIC très serrés
        i = 0
        while i < len(rows):
            row = rows[i]
...
def process_pdf(file_bytes: bytes, filename: str, force_ocr: bool = False):
    ...
    try:
        ...
        # ── Fallback OCR automatique si lecture standard retourne 0 transaction ─
        # Certains PDF ont du texte dans les en-têtes mais des tableaux non
        # extractibles → _pdf_has_text() renvoie True mais on n'a rien parsé.
        if not txns and not _ocr_used:
            logger.info("0 transaction en lecture standard — tentative OCR automatique")
            if _OCR_AVAILABLE:
                logger.info("OCR Fallback → Tesseract")
                pages_text  = _ocr_pdf(tmp_path)
                pages_words = []
                _ocr_used   = 'tesseract'
                bank = detect_bank(pages_text)
                ...
                logger.info("OCR Fallback → Claude Vision")
                pages_text  = _ocr_via_claude(tmp_path)
                pages_words = []
                _ocr_used   = 'claude_vision'
                if _pdf_has_text(pages_text, min_chars=20):
                    bank = detect_bank(pages_text)
                    if bank in AFRICAN_BANKS:
                        info, txns = PARSERS[bank](
                            pages_words, pages_text, _pdf_path=tmp_path)
                    else:
                        info, txns = PARSERS[bank](pages_words, pages_text)
...
def main():
    ...
    <div class="footer-bar">
      <span>💱 <span class="highlight">OFX Bridge</span> v2.3</span>
