...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words[:1]:
        for w in pw:
            ...
            if wt in ('DÉBIT', 'DEBIT', 'MOUV. DÉBIT', 'MOUV. DEBIT') and w['x0'] > 300:
                debit_x_min = max(300, w['x0'] - 15)
                debit_x_max = w['x0'] + 120
            elif wt in ('CRÉDIT', 'CREDIT', 'MOUV. CRÉDIT', 'MOUV. CREDIT') and w['x0'] > 400:
                credit_x_min = max(400, w['x0'] - 15)
                credit_x_max = w['x0'] + 120
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Si pas de montant avec zones calibrées, essai élargi
            if debit_amt is None and credit_amt is None:
                all_right = sorted([w for w in row if w['x0'] >= 370], key=lambda x: x['x0'])
                num_blocks = []
                cur = []
                prev_x1 = None
                for w in all_right:
