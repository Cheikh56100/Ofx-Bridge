...
def parse_cbao(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # Mémo lignes suivantes
            # IMPORTANT : une ligne sans date mais avec un montant en zone droite
            # est une NOUVELLE TRANSACTION (ex: "VIREMENT W Cartes ess / CBAO SIEGE…")
            # et non un mémo de la ligne courante.
            memo_parts = []
            continuation_txns = []   # transactions découvertes dans les lignes de suite
            j = i + 1
            while j < len(rows):
                r2 = rows[j]
                # Arrêt si une nouvelle date démarre
                if r2 and any(w['x0'] < 80 and re.match(r'^\d{2}/\d{2}/\d{4}$', w['text']) for w in r2):
                    break
                # Détecter si cette ligne de suite porte elle-même un montant en zone droite
                r2_right = sorted([w for w in r2 if w['x0'] >= 410 and re.search(r'\d', w['text'])],
                                  key=lambda w: w['x0'])
                has_amount_right = bool(r2_right)

                nl_label = ' '.join(w['text'] for w in r2 if 150 <= w['x0'] < 410).strip()

                if has_amount_right and nl_label:
                    # C'est une transaction séparée sur la même date — la parser indépendamment
                    r2_blocs, r2_cur, r2_prev_x1 = [], [], None
                    for w in r2_right:
                        is_num = bool(re.match(r'^[\d\s,\.]+$', w['text']) and re.search(r'\d', w['text']))
                        if not is_num:
                            if r2_cur: r2_blocs.append(r2_cur); r2_cur = []
                            r2_prev_x1 = None; continue
                        if r2_prev_x1 is not None and (w['x0'] - r2_prev_x1) > 25:
                            if r2_cur: r2_blocs.append(r2_cur)
                            r2_cur = [w]
                        else:
                            r2_cur.append(w)
                        r2_prev_x1 = w.get('x1', w['x0'] + len(w['text']) * 7)
                    if r2_cur: r2_blocs.append(r2_cur)
                    ...
                    for b in r2_blocs:
                        v = _uba_join_amount(b)
                        if v is not None:
                            r2_resolved.append((v, b[0]['x0']))

                    r2_debit = r2_credit = None
                    CBAO_SOLDE_MIN  = 560
                    CBAO_DEBIT_MAX  = 510
                    nl_up = nl_label.upper()
                    if r2_resolved:
                        non_solde_r2 = [(v, x) for v, x in r2_resolved if x < CBAO_SOLDE_MIN]
                        if len(non_solde_r2) == 1:
                            val_r2, x0_r2 = non_solde_r2[0]
                            is_credit_r2 = any(k in nl_up for k in CREDIT_KW)
                            is_debit_r2  = any(k in nl_up for k in DEBIT_KW)
                            if is_credit_r2 and not is_debit_r2:
                                r2_credit = val_r2
                            elif is_debit_r2 and not is_credit_r2:
                                r2_debit = val_r2
                            else:
                                if x0_r2 > 520:
                                    r2_credit = val_r2
                                    ...
                                    r2_debit = val_r2
                        elif len(non_solde_r2) >= 2:
                            c_left_r2  = min(non_solde_r2, key=lambda x: x[1])
                            c_right_r2 = max(non_solde_r2, key=lambda x: x[1])
                            r2_debit  = c_left_r2[0]
                            r2_credit = c_right_r2[0]

                    # Accumuler comme transaction à émettre après
                    date_ofx_r2 = date_full_to_ofx(date_str)
                    r2_name, r2_memo = smart_label(nl_label, [])
                    if r2_debit and r2_debit > 0:
                        continuation_txns.append(_make_txn(date_ofx_r2, -r2_debit, r2_name, r2_memo))
                    elif r2_credit and r2_credit > 0:
                        continuation_txns.append(_make_txn(date_ofx_r2, r2_credit, r2_name, r2_memo))
                else:
                    # Pas de montant à droite → c'est vraiment un mémo de la ligne courante
                    if nl_label and not any(s in nl_label.upper() for s in SKIP):
            ...
            if debit_amt and debit_amt > 0:
                ...
                txns.append(_make_txn(date_ofx, credit_amt, name, memo))
            # Émettre les transactions détectées sur les lignes de continuation
