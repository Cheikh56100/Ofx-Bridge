...
def parse_nsia(pages_words, pages_text, _pdf_path=''):
    ...
    for pw in pages_words:
        ...
        while i < len(rows):
            ...
            # IMPORTANT : :-PURCHASE = toujours un crédit à conserver, même petit montant.
            is_tpe_frais = False
            if 'SV CLEARING' in label_up or 'CLEARING' in label_up:
                # Jamais filtrer les :-PURCHASE (crédits TPE)
                is_minus_purchase = (':-PURCHASE' in label_up
                                     or '-PURCHASE' in label_up
                                     or bool(re.search(r':-\s*PURCHASE', label_up)))
                if not is_minus_purchase:
                    if debit_amt and debit_amt < 5000:
                        is_tpe_frais = True
                    elif credit_amt and credit_amt < 5000:
                        is_tpe_frais = True
    ...
    if not any(t for t in txns if t and 'CLEARING' in t.get('name','').upper()):
        ...
        for idx, line in enumerate(lines_nsia):
            ...
            # :-PURCHASE (avec ou sans espace autour du tiret) = crédit TPE
            _has_minus_purchase = bool(re.search(r':-\s*PURCHASE|-\s*PURCHASE', label_up_raw))
            is_credit_raw = (_has_minus_purchase or
                             any(k in label_up_raw for k in CREDIT_KW))
            is_debit_raw  = (':PURCHASE' in label_up_raw and not _has_minus_purchase)
