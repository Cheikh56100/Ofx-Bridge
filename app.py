...
def _extract_sg_header(pages_text):
    ...
    # pdfplumber peut lire "n° 30003 03320 00020641644 69" ou "n°30003 03320 ..."
    if not info['iban']:
        rib_m = re.search(
            r'n[°o]\s*(\d{5})\s+(\d{5})\s+(\d{10,12})\s+(\d{2})\b',
            text
        )
        if rib_m:
            banque  = rib_m.group(1)   # ex: 30003
            guichet = rib_m.group(2)   # ex: 03320
            compte  = rib_m.group(3)   # ex: 00020641644
            cle     = rib_m.group(4)   # ex: 69
            info['_rib_bank']    = banque
            info['_rib_agency']  = guichet
            info['_rib_account'] = compte
            info['_rib_key']     = cle
            # Construire un pseudo-IBAN lisible pour l'affichage dans l'UI
