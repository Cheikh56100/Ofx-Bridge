...
def _afr_header(pages_text):
    ...
    # ── 4. Si IBAN trouvé mais pas de RIB, dériver le RIB depuis l'IBAN ──────
...
def parse_bsic(pages_words, pages_text, _pdf_path=''):
    ...
    # ── Extraction clé RIB depuis "Numéro de compte : BBBBB-CCCCCCCCCCC-KK" ──
    # Format BSIC : "01001-00100029193-76"  (Guichet-Compte-Clé)
    m_compte = re.search(
        r'Num[eé]ro\s+de\s+compte\s*:\s*(\d{5})-(\d{8,14})-(\d{2})',
        full_text, re.IGNORECASE
    )
    if m_compte and not info.get('_rib_key'):
        # Compléter/corriger les champs RIB
        info['_rib_agency']  = m_compte.group(1)
        info['_rib_account'] = m_compte.group(2)
        info['_rib_key']     = m_compte.group(3)
        if not info.get('_rib_bank') and info.get('iban'):
            # Dériver le code banque depuis l'IBAN
            iban_c = re.sub(r'\s+', '', info['iban']).upper()
            info['_rib_bank'] = iban_c[4:9]  # 5 chars après CC+KK
...
def generate_ofx(info, txns, target='quadra', currency='EUR'):
    ...
    # ── ACCTID : limiter à 22 caractères (limite OFX SGML) ───────────────────
...
def main():
    ...
    <div class="footer-bar">
      <span>💱 <span class="highlight">OFX Bridge</span> v2.2</span>
