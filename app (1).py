...
def extract_iban(text):
    ...
    # ── 3. Fallback UEMOA/BCEAO sans mot-clé (scan global) ────────────────────
    ...
    if m3:
        result = _clean_and_truncate(m3.group(1))
        if result:
            return result
...
def generate_ofx(info, txns, target='quadra', currency='EUR'):
    ...
    # ── ACCTID pour Quadra ────────────────────────────────────────────────────
    # Cas 1 : IBAN réel (commence par 2 lettres + 2 chiffres) → on l'utilise compact
    # Cas 2 : RIB SG sans IBAN → concaténer Banque+Guichet+Compte+Clé
    # Cas 3 : fallback sur le fragment RIB extrait par iban_to_rib()
    iban_is_real = bool(re.match(r'^[A-Z]{2}\d{2}', iban_full.replace(' ','')))
    if iban_is_real:
        acctid = re.sub(r'\s+', '', iban_full)
    elif info.get('_rib_account'):
        acctid = (info.get('_rib_bank','') +
...
def main():
    ...
    for uploaded_file in uploaded_files:
        ...
        if iban_value != iban_detected:
            ...
            # Détecter si c'est un RIB (5+5+11+2) ou un IBAN (commence par 2 lettres)
            rib_parts = re.match(r'^(\d{5})\s+(\d{5})\s+(\d{10,12})\s+(\d{2})$', iban_clean)
            if rib_parts:
                info['_rib_bank']    = rib_parts.group(1)
                info['_rib_agency']  = rib_parts.group(2)
                info['_rib_account'] = rib_parts.group(3)
                info['_rib_key']     = rib_parts.group(4)
        ...
        if iban_display != '—':
            st.markdown(
                f"<div style='font-size:0.78rem; color:#7489b0; margin-top:-0.4rem; margin-bottom:0.8rem;'>"
                f"🔑 Utilisé dans l'OFX&nbsp;: <code style='color:#1a56db'>{iban_display}</code></div>",
                unsafe_allow_html=True
            )
